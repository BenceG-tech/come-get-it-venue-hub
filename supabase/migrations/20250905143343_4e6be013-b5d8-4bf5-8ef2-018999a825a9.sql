...
-- 1) Add the missing column expected by the mobile client
ALTER TABLE public.venues
  ADD COLUMN IF NOT EXISTS points_per_visit integer NOT NULL DEFAULT 1;

-- 2) Recreate the public.get_public_venues RPC to include points_per_visit
DROP FUNCTION IF EXISTS public.get_public_venues(text, integer);

CREATE FUNCTION public.get_public_venues(
